...
try:
    ...
    try:
        r = wskey.get(serviceURL + "/holdingcodes/data/128807", headers={"Accept":"application/json"})
        r.raise_for_status
        result = r.json()
        holding_codes = result.get('content').get('holdingcodes').get('holdingCodeConfig')
        holdingCodeInfo = list(map(lambda row : {"branchId": row.get('branchRegistryId'), "holdingCode": row.get('holdingCode'), "displayName": row.get('displayName')}, holding_codes))
        print(holdingCodeInfo)
