...
# when specifying a particular institution to authenticate at 
...
def getToken():
    if request.args.get('error'):
        ...
        #redirect to a specific institution for login   
        authorization_url, state = app.config.get('oauth_session').authorization_url(app.app_config.get('auth_url')+ "/128807")
...
def getBib(): 
    try:
        ...
        r = oauth_session.get(app.app_config.get('wms_availability_url') + "/availability/sru/service?x-registryId=128807" + + "&query=no:932367058", headers={"Accept":'application/xml'})
        r.raise_for_status
        xml = r.content
        return xml
