...
serviceURL = config.get('lman_service_url')   
# get a token
scope = ['WMS_LMAN']
...
license_id = '491eb1bb-7b98-4d13-9517-0bdea9ed2db2'

try:
    token = wskey.fetch_token(token_url=config.get('token_url'), auth=auth) 
    try:
        r = wskey.get(serviceURL + "/license/data/" + licenseId, headers={"Accept":"application/json"})
        r.raise_for_status
        result = r.json()
        print(result)
